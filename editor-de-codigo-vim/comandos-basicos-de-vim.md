...
* Este comando es muy poderoso, pero aquí simplemente lo introducimos de manera muy básica.
* Sustituye toda ocurrencia del patrón `<PATTERN>` por la cadena `<REPLACEMENT>`.
  * Por ejemplo: `:%s|int|double|g` reemplaza toda ocurrencia de la cadena `int` por la cadena `double`.
...
**`w`** -> avanzar al principio de la siguiente palabra (**`25 w`** -> avanzar 25 palabras).

**`gg`** -> Posicionar el cursor en la primera línea del buffer.
